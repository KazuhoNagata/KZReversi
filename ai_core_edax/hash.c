...
void hash_code_init(void)
{
	...
	for (i = 0; i < 16; ++i)
	{
		for (j = 0; j < 256; ++j)
		{
			do
			{
				hash_rank[i][j] = genrand_int32();
				hash_rank[i][j] |= (UINT64)genrand_int32() << 32;
...
{
	if (USE_TYPE_PUNING)
	{ // HACK: depends on little endian.
		return (*(unsigned int*)data);
	}
	else
	{ // slow but more portable implementation.
